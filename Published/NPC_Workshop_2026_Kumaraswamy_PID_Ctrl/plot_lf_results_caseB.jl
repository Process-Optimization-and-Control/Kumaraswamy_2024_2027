function plot_lf_case_results(case_name)
    ...
    # SETPOINT DATA   
    ####################################################
    # NOTE: these overlays must mirror the SETPOINT SCHEDULE in ctrl_scheme_B.jl.
    # Case B now runs the same schedule as Case D (only the well tuning differs),
    # so the reference lines below are identical to plot_lf_results_caseD.jl.
    ...
    plot!(plt9, time, wellhead_flows[:, 1],
                    label = "Well 1")
    ...
    # Keep this list identical to plot_lf_results_caseA/D.jl so that plt_N.png means
