...
#
# Offset-free layer     : ON. Additive bias on the 4 well flow measurements and on the
#                         pressures at the constrained nodes. The mainline flow bias and the
#                         valve-CV / injectivity estimators are OFF (see the settings below).
...
#
# Companion case: case_final_no_offset.jl - identical apart from the bias terms being switched off.
