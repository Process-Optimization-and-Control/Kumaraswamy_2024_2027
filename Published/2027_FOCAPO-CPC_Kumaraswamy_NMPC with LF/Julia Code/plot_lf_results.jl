...
####################################################
# CLOSED-LOOP RESULT PLOTS
#
# Plots the LedaFlow (plant) trends for one completed case and writes them to <case_dir>/lf_plots.
# This is a script, not a function: include it after a case file, from plot_lf_results.ipynb.
# It requires full_lf_output.csv, which extract_full_output.jl produces at the end of a run.
#
# Everything here is plant data. To plot what the controller predicted instead, use plot_pred.jl.
####################################################
...
# Sign flipped to match the injection direction, as in run_sim.jl
...
# Phase boundaries: times where any well's setpoint changes value. Only well 4 changes at the
# second boundary, which is why the phases are read off all four setpoint vectors and not just one.
...
function annotate_sp_phases(plt_annotate; fs = 16, y_offset=-0.1)
    # Overlay the setpoint phase structure on a plot: a marker for the instant the controller is
    # switched on, a P1..Pn label per phase, and alternating shading to separate them.
    # `fs` is the phase-label font size: 
    # `y_offset` places the labels as a fraction of the y-range below the top of the axis.
...
# Default Plot Settings. Applied to every plot below via Plots.default, so individual plots only
# set what differs. Computer Modern and dpi=750 are for figures going into the paper.
tick_positions = 0:7200:time[end]  # ticks every 2 hours
# Axis is in seconds but labelled in hours
...
####################################################
# CONSTRAINTS
# The two constrained pressures: node 2 (pump outlet) and nodes 52/54/65/67 (bottom hole).
# The limits are drawn from controller_on_time onwards only, since they are not enforced during
# the LedaFlow initialisation period.
####################################################
...
pump_outlet_press = df[!, "Pressure@Mainline P 500m"]  # transmitter 500 m into the mainline, used as the pump outlet
...
plot!(plt4, [controller_on_time, time[end]], fill(pipe_pressure_max[52], 2),
                ...
                color = :red,
                label = "High Limit")
# The lower bottom-hole limit is far below the operating range and never active, so it is left off
# the figure to keep the y-axis on the region of interest.
...
####################################################
# INLET PRESSURES AT MAINLINE, LINE 1, LINE 2
####################################################
...
linestyle = :dot,
# Wells 1-3 share the same setpoint trajectory and are drawn in one colour; well 4 is the one that
# differs in the middle phase, so it gets its own colour to make the split visible.
...
# The panels reuse the plot objects built above, so they already carry their phase annotations.
...
####################################################
# JOIN PLOTS 1 & 2 (Choke Valve Openings and Pump Speed) AS ONE FIGURE
####################################################

# The two manipulated variables as one 2-panel figure
plt12 = plot(plt1, plt2, layout = (1, 2), size = (1200, 500),
left_margin = 10Plots.mm, right_margin = 10Plots.mm,
top_margin = 10Plots.mm, bottom_margin = 10Plots.mm)


# Saved as plt_1 .. plt_11 in the order listed here
