...
##############################
original_dir = pwd()
CASE_NAME = "04_max_throughput_opt"
...
function special_constraints_fn_dynamic(model, sim_t)
    ...
    ##############################
    @objective(model, Min, 
    sum(100*model[:pump_work][1, t] for t=2:T_mpc) # Minimise pump work done 
    - 5*sum(model[:w_in][51, t] for t=2:T_mpc) # Maximise Well Flow Rate 
    - 20*sum(model[:w_in][53, t] for t=2:T_mpc) #   Maximise Well Flow Rate
    - 80*sum(model[:w_in][64, t] for t=2:T_mpc) #   Maximise Well Flow Rate
    - 200*sum(model[:w_in][66, t] for t=2:T_mpc) # Maximise Well Flow Rate
