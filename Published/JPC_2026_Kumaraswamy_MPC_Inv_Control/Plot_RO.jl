...
function plot_save_results(fplt_ylim, hplt_ylim, Hprofile, Fprofile, alpha_tk, original_dir)
    ...
    storage_tk_labels = [1, 2, 3, 4, 5]
    hlabels = [L"h_{%$i}" for i in storage_tk_labels]
    hlabels = permutedims(hlabels)

    plt_fmin, plt_fmax = fplt_ylim
    plt_hmin, plt_hmax = hplt_ylim

    default(legendfont=(12, "times"), tickfont=(12, "times"), guidefont=(14, "times"))
    plt1 = plot(1:nt, [Fprofile[:,i] for i in 1:O],
        xlabel=L"Time\,(hr)", ylabel=L"F\,(m^{3}/hr)", lw=2.5, 
        label=flabels, ylimits = (plt_fmin, plt_fmax),
        yformatter=:plain, legend=:outerright, linetype=:steppre, dpi=750)
    plt2 = plot(1:nt+1, [Hprofile[:,i]/hcap[i] for i in storage_tk_idx], 
        xlabel=L"Time\,(hr)", ylabel=L"Level\,(\%)", lw=2.5, 
    ...
    for k in storage_tk_labels
        i = storage_tk_idx[k]
        plt = plot(1:nt+1, [Hprofile[:,i]/hcap[i]],
        xlabel=L"Time\,(hr)", ylabel=L"Level\,(\%)", lw=2.5,
        label=L"h_{%$k}", ylimits=(plt_hmin, plt_hmax),
        yformatter=:plain, legend=:outerright, linetype=:steppre, dpi=750)
        display(plt)
