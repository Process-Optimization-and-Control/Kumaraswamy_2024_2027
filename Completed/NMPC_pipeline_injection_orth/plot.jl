...
####################################################
# Every state carries a collocation dimension d; the plots always take the last collocation point
# d = D (`dsel` below), i.e. the value at the end of each finite element, which is the sample the
# closed-loop result is reported at. The inputs have no collocation dimension and are indexed
# directly.
...
function plot_save(results)
    ...
    dsel = last(axes(results["w_in"])[end-1])
    data4 = results["w_in"][Name.(well_idx), dsel, :]'
    ...
    # PIPELINE PRESSURE RATE OF CHANGE AT THE CONSTRAINED NODES
    # Same backward difference as the soft constraint in model_mpc.jl; the first sample has no
    # predecessor, hence the leading NaN column.
    ...
    for i in pump_idx
        ...
        dsel = last(axes(results["pump_work"])[end-1])
        pump_work_data = results["pump_work"][Name(i), dsel, :]
    ...
    function plot_data(x)
        ...
        # Draw the cavitation index low limit on the cavitation index plot
        if plt_number == 7
