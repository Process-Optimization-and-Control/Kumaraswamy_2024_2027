...
function special_constraints_fn_dynamic(model, sim_t)
    ...
    @objective(model, Min,
    - 5*sum(model[:w_out][51, D, t] for t=1:T_mpc) # Well Flow Rate Setpoint
    - 20*sum(model[:w_out][53, D, t] for t=1:T_mpc) # Well Flow Rate Setpoint
    - 80*sum(model[:w_out][64, D, t] for t=1:T_mpc) # Well Flow Rate Setpoint
    - 200*sum(model[:w_out][66, D, t] for t=1:T_mpc) # Well Flow Rate Setpoint
