...
##############################
# ORTHOGONAL COLLOCATION PARAMETERS
##############################
# The dynamics are discretised by orthogonal collocation on finite elements. Every finite element
# is one sample of length DT and carries D points along the collocation dimension d:
#   d = 1     the start of the element. It holds the initial condition
#   d = 2:D   the interior collocation roots and the end of the element. d = D is the end point,
#             i.e. the value reported as the result for that sample
# Adot is the collocation derivative matrix: Adot'*x[1:D] gives the scaled time derivative of x at
# the collocation points d = 2:D, which is what the mass balance is written against in
# model_mpc.jl and model_sim.jl.
D = 4
Adot =
