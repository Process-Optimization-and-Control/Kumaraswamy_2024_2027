...
# CASE DESCRIPTION
##############################
# CASE 1b: Case B in IFAC Conference Paper
...
function special_constraints_fn_dynamic(model, sim_t)
    ...
    + 5*sum((model[:w_in][51, D, t] - well_1_flow_sp_mpc[t])^2 for t=1:T_mpc) # Well Flow Rate Setpoint
    + 20*sum((model[:w_in][53, D, t] - well_2_flow_sp_mpc[t])^2 for t=1:T_mpc) # Well Flow Rate Setpoint
    + 80*sum((model[:w_in][64, D, t] - well_3_flow_sp_mpc[t])^2 for t=1:T_mpc) # Well Flow Rate Setpoint
    + 200*sum((model[:w_in][66, D, t] - well_4_flow_sp_mpc[t])^2 for t=1:T_mpc) # Well Flow Rate Setpoint
