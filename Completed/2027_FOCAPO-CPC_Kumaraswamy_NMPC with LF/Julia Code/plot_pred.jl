####################################################
# NMPC PREDICTION PLOTS
#
# Plots one saved MPC prediction, i.e. what the controller expected to happen over the prediction
# horizon at a single sample. Takes the dictionary stored in <case_dir>/mpc_pred/pred_t<t>.jld2 by
# run_mpc.jl, so the x-axis spans one horizon (DT..NP), not the whole closed-loop run.
# For the plant trends over the whole run, use plot_lf_results.jl instead.
#
# Each plot is first described as a NamedTuple of data plus axis settings, and the plot_data /
# plot_bar function at the bottom render and save them all in one pass.
####################################################
...
function plot_save(results)
    ...
    # dsel picks the last collocation point, d = D, i.e. the value at the end of each sample. It is
    # read from the variable's own axes because d starts at 2 for most variables and at 1 for others.
    ...
    # Bias-corrected well flows: the quantity the objective actually tracks
    ...
    # Inputs have no collocation dimension, so they are indexed by arc and sample only
    ...
    # Diagnostic only: how fast the constrained node pressures move along the predicted horizon.
    # There is no pressure rate-of-change constraint in model_mpc.jl; the rate limits are on the
    # inputs. Backward difference, so the first sample has no value and is padded with NaN.
    pressure_roc = hcat(
    ...
    for i in pump_idx
        ...
        # Work is carried entirely at the endpoint collocation node (the interior ones are fixed to
        # zero in model_mpc.jl), so summing over d gives the total work over the sample.
        pump_work_data = vec(sum(results["pump_work"][Name(i), :, :], dims=1))
    ...
    # Marks which of the pump plots is drawn as a bar chart: the work per sample, since it is a
    # quantity accumulated over each interval rather than an instantaneous value. One entry per
    # plot per pump, in the push! order above.
    pump_plots_bar_indicator = repeat([1, 0, 0, 0], length(pump_idx))

    # NOTE: there is no plt8; the numbering skips it. The cavitation plot is plt7 and is also the
    # 7th entry here, which the plt_number == 7 test in plot_data relies on.
    plots = [plt1, plt2, plt3, plt4, plt5, plt6, plt7, plt9, plt10, plt11, plt12, plt13, plt14]

    # MOVE INTO RESULTS DIRECTORY - figures are saved by relative name below, and cd(original_dir)
    # at the end of the function puts the working directory back.
    mkpath(joinpath(@__DIR__, CASE_NAME))
    cd(case_dir)

    # Renders one plot description, saves it as <CASE_NAME>_plt<n>.png and advances the counter.
    function plot_data(x)
        tick_positions = DT:3600:NP  # x-axis is in seconds; ticks every hour, labelled in hours
       ...
       size=plot_size,
        grid=false)
        if plt_number == 7
            # plt7 is the cavitation index; overlay its lower limit
    ...
    # As plot_data, but for quantities accumulated over each sample rather than sampled at a point.
    function plot_bar(x)
        tick_positions = DT:3600:NP  # ticks every hour, labelled in hours
        tick_labels = string.(round.(Int, tick_positions ./ 3600))
