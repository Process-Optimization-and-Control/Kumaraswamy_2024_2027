...
# is no collocation and no time dimension (d and t are both singletons, kept only so that the
# variable shapes match). It is solved once at the start of a run and serves two purposes:
#   1. its pump speed and choke openings are the settings LedaFlow is initialised with (lf_ss.jl)
#   2. its full solution acts as an initial guess for the first NMPC solve (case notebook)
...
function ss_model_build_solve()
    ...
    @variable(model, q_pump[k in pump_idx, d=[1], t=1:T_ss]>=0)
    ...
    @variable(model, choke_vlv_op[k in all_vlv_idx, t=1:T_ss]>=0, start=1.0)
    @variable(model, speed_pump[k in pump_idx, t=1:T_ss]>=0, start=3440)
    ...
    @constraint(model, [k in pump_idx, d=[1], t=1:T_ss], pump_work[k, d, t] == ( (p_out[k, d, t] - p_in[k, d, t])*(10^5)*w_in[k, d, t]/pump_efficiency[k]/(0.5*(rho_pump_inlet[k, d, t] + rho_pump_outlet[k, d, t])) )*DT/(3.6e9) ) # Shaft work in MWh over one DT interval; /3.6e9 converts W.s to MWh
    ...
    # STEADY STATE EQUATION - no accumulation, so what enters a pipe segment also leaves it.
    # This is what replaces the collocation mass balance of model_mpc.jl.
    ...
    ##############################  
    # UNCOMMENT BELOW TO PRINT ALL VARIABLE VALUES
    ##############################
